...
class Database:
    ...
    * method close() closes connection to db
    ...
    def update(self, local_param, local_dir, 
               email_list, excel: 'Excel'):
        """Update_database."""

        fullname, filename = email_list

        update_db_list = self.filing(local_param,
                                         fullname,
                                         excel)


        drop_table = f"""DROP TABLE {local_dir}"""
        self.cur.execute(drop_table)

        self.create(local_dir,
                    update_db_list,
                    local_param)
