...
SQL_REQUEST = {
                  ...
                  'Закупка': [SQL_PURCHASE, SQL_PURCHASE_IN, SQL_PURCHASE_OUT],
...
# Const_dir
SUP_DIR = ['Статусы', 'Закупка', 'Закупка2', 
...
PARAM = [{
            ...
            'MAIN_KEY': 'TEXT, ',
            'CЕБЕСТОИМОСТЬ ЕДИНИЦЫ БЕЗ НДС': 'TEXT, ',
            'PK': 'ID INTEGER PRIMARY KEY'
    },
            {
            'PN': 'TEXT, ',
...
# Body_text
BODY = ('Файл {name} не подходит для обработки',
        'Обработанный файл {name} во вложении.',
        'Database не обновлена.',
        'Database обновлена, таблица {tablename} добавлена, файл {filename} корректный')
...
def create_db(table: list, param: list, 
    ...
    format_xlsm = ('Закупка1', 'Закупка2')
    try:
        for name in table:
                ind = table.index(name)
                local_dir = path.join(ROOT_DIR, name)
                print(local_dir)
                ...
                xlsx_list = db.filing(param[ind], root[0][0], excel)
                db.create(name, xlsx_list, param[ind])
    except:
        raise 'Error create Database.'

def main(db, write, ebay, excel, filee):    
    """Main_func."""
    ...
    # Connect_outlook_CROC
    try:
        email = Email(USERNAME_OUTLOOK_CROC, PASSWORD_OUTLOOK_CROC)
        if not email.find_items_in_message(filee):
            return
    except:
        return

    if email.UPDATE_INDEX_DB:
            
        fulldir, filename = email.FILE_LIST[0]

        try:
            db.update(PARAM[email.UPDATE_INDEX_DB],
                     SUP_DIR[email.UPDATE_INDEX_DB],
                     (fulldir, filename),
                     excel)
        except:
                email.MESSAGE_SUBJECT = 'ERROR ALLERT DATABASE'
                email.RECIPIENTS = email.RECIPIENTS[:1]  
                email.send_email(attachments=(fulldir, filename))   
                remove(fulldir)

        local_dir = path.join(ROOT_DIR, SUP_DIR[email.UPDATE_INDEX_DB])
            
        filee.delete_cache([local_dir], email.FILE_LIST[0][1])
            
        email.MESSAGE_BODY = BODY[3].format(tablename=SUP_DIR[email.UPDATE_INDEX_DB],
                                            filename=filename)
        email.RECIPIENTS = email.RECIPIENTS[:1]
        email.send_email()
        return         

    for fulldir, filename in email.FILE_LIST:
                
        try:

            xlsx_list = excel.find_item(SAMPLE_XLSX, fulldir)
                
            if not xlsx_list:
                email.MESSAGE_BODY = BODY[0].format(name=filename)
                email.send_email()
                remove(fulldir)
                continue
            ...
            for item in xlsx_list:
                ...
                email.MESSAGE_BODY = BODY[1].format(name=filename) + data['SHEET']
                email.send_email(attachments=(write.OUTPUT_FILE, filename))
                remove(write.OUTPUT_FILE)
            
            remove(fulldir)
                
        except:
            email.MESSAGE_SUBJECT = 'ERROR ALLERT'
            email.RECIPIENTS = email.RECIPIENTS[:1]  
            email.send_email(attachments=(fulldir, filename))   
            remove(fulldir)
            if filee.find_file(INBOX_DIR[1]):
                remove(write.OUTPUT_FILE)
...
if __name__ == '__main__':
    ...
    while(True):
        ...
        try:
            main(db, write, ebay, excel, filee)
        except:
            filee.delete_cache(INBOX_DIR)
            continue
