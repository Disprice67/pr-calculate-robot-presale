...
class File:
    ...
    * method find_file() helps to find the desired file 
    ...
    def delete_cache(self, input_dir, filename: None):
        """remove_file_in_folder"""
        for dirs in input_dir:
            files = self.find_file(dirs)
            if files:
                for fullname, file_name in files:
                    if filename:
    ...
    def find_file(self, folder: str = None,
        ...
        for file in glob.glob(f'*{format}'):
            file_full = os.path.join(folder, file)
...
class Excel:
    ...
    * method exceptions() handling exceptions and generating a list of party members
    ...
    # filter_collision
    FILTER_COL: tuple = ('АРТИКУЛ', 'PN', 'МОДЕЛЬ НАЧИНАЕТСЯ С…', 'PART #', 'КОД ПРОЕКТА')
...
class Writer(File):
    ...
    * method setconst() extract constants from excel
    ...
    # root_dir
    OUTPUT_FILE: str =  os.path.join(OUTPUT_DIR, 'end.xlsx')

    def __init__(self):
        self.file = self.find_file(SAMPLE_DIR)

    def writeinfo(self, data):
        """Write_info."""

        sample_book = load_workbook(self.file[0][0], data_only=False)
